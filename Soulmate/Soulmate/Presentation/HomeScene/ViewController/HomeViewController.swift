...
class HomeViewController: UIViewController {
    ...
    private let heart = UIImageView().then {
        $0.image = UIImage(named: "heart")
        $0.frame = CGRect(x: 0, y: 0, width: 17.07, height: 14.06)
        $0.contentMode = .scaleAspectFit
    }
    
    private let numOfHeartLabel = UILabel().then {
        $0.text = "00"
    ...
    // MARK: - 후보1 UI
    private let candidateUpperName = UILabel().then {
        $0.text = "초록잎"
    ...
    private let candidateUpperAge = UILabel().then {
        $0.text = "25"
    ...
    private let candidateUpperMapImageView = UIImageView().then {
        $0.image = UIImage(named: "mapGrey")
        $0.frame = CGRect(x: 0, y: 0, width: 13.04, height: 18)
        $0.contentMode = .scaleAspectFit
    }
    
    private let candidateUpperDistance = UILabel().then {
        $0.text = "3 km"
    ...
    // MARK: - 후보2 UI
    private let candidateLowerName = UILabel().then {
        $0.text = "파란잎"
        $0.frame = CGRect(x: 0, y: 0, width: 58, height: 26)
        $0.textColor = UIColor(red: 1, green: 1, blue: 1, alpha: 1)
        $0.font = UIFont(name: "AppleSDGothicNeo-Bold", size: 22)
    ...
    private let candidateLowerAge = UILabel().then {
        $0.text = "21"
        $0.frame = CGRect(x: 0, y: 0, width: 23, height: 26)
        $0.textColor = UIColor(red: 1, green: 1, blue: 1, alpha: 1)
        $0.font = UIFont(name: "AppleSDGothicNeo-Light", size: 22)
    ...
    private let candidateLowerMapImageView = UIImageView().then {
        $0.image = UIImage(named: "mapGrey")
        $0.frame = CGRect(x: 0, y: 0, width: 13.04, height: 18)
        $0.contentMode = .scaleAspectFit
    }
    
    private let candidateLowerDistance = UILabel().then {
        $0.text = "21 km"
        $0.frame = CGRect(x: 0, y: 0, width: 32, height: 20)
        $0.textColor = UIColor(red: 1, green: 1, blue: 1, alpha: 1)
        $0.font = UIFont(name: "AppleSDGothicNeo-Medium", size: 15)
    ...
    private let candidateStackView = UIStackView().then {
        $0.spacing = 16
        $0.axis = .vertical
        $0.alignment = .fill
    ...
    // MARK: - 다시 추천 버튼
    private let recommendAgainButton = UIButton(type: .system).then {
    ...
    private func addSubviews() {
        ...
        view.addSubview(numOfHeartLabel)
        candidateStackView.addArrangedSubview(candidateUpperView)
        candidateStackView.addArrangedSubview(candidateLowerView)
        view.addSubview(candidateStackView)
        view.addSubview(recommendAgainButton)
    }
